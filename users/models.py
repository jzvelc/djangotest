...
def generate_avatar_filename(instance, filename):
    ...
    return 'avatars/%s%s' % (hashlib.md5(f.encode('utf-8')).hexdigest(), ext)
...
class WindsurfProfile(models.Model):
    BEGINNER = 'B'
    INTERMEDIATE = 'I'
    ADVANCED = 'A'
    LEVELS = (
    ...
    experience = models.CharField(max_length=1, choices=LEVELS, default=INTERMEDIATE)

    board_volume = models.PositiveIntegerField()
    board_width = models.PositiveIntegerField()
    board_length = models.PositiveIntegerField()
...
class User(AbstractBaseUser, PermissionsMixin):
    ...
    email = models.EmailField(
        _('Email address'),
        help_text=_('Required. Email must be unique since it identifies user.'),
        max_length=254,
        unique=True,
        db_index=True
    )

    profile = models.ForeignKey(WindsurfProfile, null=True, blank=True, related_name='user')  # consider polymorphic relation
